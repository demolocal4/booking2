    ...
    <div class="card">
       <x-booking-counter />
        ...
        <div class="card-body">
            ...
            {{-- list view ------------------------------------------------------------------}}
            <div class="list-view" style="@if(isset($_GET['page']) || isset($_GET['search_from'])) display:block; @else display:none; @endif">
            {{-- new filter for list view added on 12-04-2023            --}}
            <form action="{{ url('room_types') }}" id="fmFilter1" method="GET">
                <input type="hidden" name="view" value="calendar">
                <div class="row mb-5 mt-5">
                    <div class="col-md-3 offset-2">
                        <div class="form-grop">
                        <select name="search_from" id="search_from" class="form-select" required>
                            <option value="" disabled selected>Search From</option>
                            <option value="roomType"
                            @if(isset($_GET['search_from']) && $_GET['search_from'] == 'roomType')
                            selected
                            @endif
                            >Room Type</option>
                            <option value="brCode"
                            @if(isset($_GET['search_from']) && $_GET['search_from'] == 'brCode')
                            selected
                            @endif
                            >Branch Code</option>
                        </select>
                    ...
                    <div class="col-md-3">
                        <div class="form-grop">
                        <input type="text" name="search_with" id="search_with" value="{{ request('search_with') ?? '' }}" class="form-control" required>
                        </div>
                    </div>
                    <div class="col-md-2">
                        <button type="submit" class="btn btn-outline-dark">Search</button>
                        <a href="{{ url('room_types') }}" class="btn btn-outline-warning">Clear</a>
                    </div>
                </div>
            </form>    
            {{-- new filter for list view added on 12-04-2023            --}}    	
...
<!--Disabled Backdrop Modal update-->
  ...
  @endforeach
            ...
            {{-- calendar view ------------------------------------------------------------------}}
            <div class="calendar-view" style="@if(isset($_GET['page']) || isset($_GET['search_from'])) display:none; @else display:block; @endif">
            ...
            <form action="{{ url('room_types') }}" id="fmFilter" method="GET">
               ...
               {{-- <section class="regular slider">
                ...
                @foreach($data->unique('roomtype_id') as $cardRow)
                <section class="regular slider">
                        @foreach($data->where('roomtype_id', $cardRow->roomtype_id)->sortBy('id') as $eachType)
                        <div>
                            <div class="box-inner" id="box_{{ $eachType->id }}">
                            <h5>{{ date('d', strtotime($eachType->date)). ' ' .date('F', strtotime($eachType->date)) }}</h5>
                            <p class="card-text">{{ date('l', strtotime($eachType->date)) }}</p>
                            <p class="card-text">{{ $eachType->roomType }}</p>
                            <p class="card-text">{{ $eachType->branch->brName }}</p>
                            <table style="width:90%;margin:-8px auto;">
                              <tr>
                                  <th>Regular:</th>
                                  <td>{{ 'AED '.$eachType->regular }}</td>
                              </tr>
                              <tr>
                                  <th>Weekly:</th>
                                  <td>{{ 'AED '.$eachType->weekly }}</td>
                              </tr>
                              <tr>
                                  <th>Monthly:</th>
                                  <td>{{ 'AED '.$eachType->monthly }}</td>
                              </tr>
                            </table>
                            @if($access->canEdit == 1)
                            <button class="btn btn-outline-dark btn-sm rate-edit mt-3" id={{ $eachType->id }}>Edit</button>
                            @endif
                            <div class="rate-update" id="rate_{{ $eachType->id }}">
                            <hr>
                            <form action="{{ url('rates_update') }}" method="POST" id="frm_"{{ $eachType->id }}>
                            @csrf
                            <input type="hidden" name="id" value="{{ $eachType->id }}">   
                            <div class="row">
                                <div class="col-md-6 col-12">Regular</div>
                                <div class="col-md-6 col-12"><input type="number" name="regular" step="0.01" min="1" 
                                    value="{{ $eachType->regular }}"
                                    class="form-control element" disabled></div>
                            </div>
                            <div class="row">
                                <div class="col-md-6 col-12">Weekly</div>
                                <div class="col-md-6 col-12"><input type="number" name="weekly" step="0.01" min="1" 
                                    value="{{ $eachType->weekly }}"
                                    class="form-control element" disabled></div>
                            </div>
                            <div class="row">
                                <div class="col-md-6 col-12">Monthly</div>
                                <div class="col-md-6 col-12"><input type="number" name="monthly" step="0.01" min="1" 
                                    value="{{ $eachType->monthly }}"
                                    class="form-control element" disabled></div>
                            </div>
                            <div class="row mt-3">
                                <div class="col-12">
                                    <button type="submit" class="btn btn-primary btn-sm">Update</button>
                                    <button type="button" class="btn btn-dark btn-sm btn_close" data-id={{ $eachType->id }}>Close</button>
                                </div>
                            </div>
                            </form>
                            </div>
            ...
            <a href="#top" id="goTop">
            <img src="{{ asset('images/back-top.png') }}">    
            </a>    
        </div>
    </div>
